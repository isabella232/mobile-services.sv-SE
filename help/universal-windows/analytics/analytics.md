...
1. Klicka på **[!UICONTROL Enable Latest App Reports]**.
   ...
   Du kan också klicka **[!UICONTROL Enable Mobile Location Tracking]** eller **[!UICONTROL Enable Legacy Reporting and Attribution for background hits]**.
...
Om `CollectLifecycleData()` anropas två gånger under samma session rapporterar programmet en krasch vid varje anrop efter det första. SDK anger en flagga när programmet stängs som anger att det har avslutats. Om flaggan inte är inställd rapporterar en krasch `CollectLifecyleData()` .
...
Om du vill ange *`products`* i mobil-SDK måste du använda en speciell syntax. Mer information finns i [Variabeln](/help/universal-windows/analytics/products.md)Produkter.

## (Valfritt) Aktivera spårning offline {#section_955B2A03EB854742BDFC4A0A3C287009}

Om du vill lagra träffar när enheten är offline kan du aktivera offlinespårning i [SDK-metodfilen](/help/universal-windows/c-configuration/methods.md) . Var noga med tidsstämpelkraven som beskrivs i konfigurationsfilreferensen innan du aktiverar spårning offline.
...
* Latitud/longitud och POI (om det finns i en POI definierad i `ADBMobileConfig.json` konfigurationsfilen).
...
När enhetsplatsen bestäms ligga inom en radie på 7000 meter från den definierade punkten, skickas en `a.loc.poi` kontextdatavariabel med värdet `San Francisco` med `TrackLocation` träffen. En `a.loc.dist` kontextvariabel skickas med avståndet i meter från de definierade koordinaterna.
