...
1. Klicka på **[!UICONTROL Enable Latest App Reports]**.
   ...
   Du kan även klicka **[!UICONTROL Enable Mobile Location Tracking]** och **[!UICONTROL Enable Legacy Reporting and Attribution for background hits]**.
...
Till *`products`* i mobil-SDK måste du använda en särskild syntax. Se [Produktvariabel](/help/windows-appstore/analytics/products/products.md).

## (Valfritt) Aktivera spårning offline {#section_955B2A03EB854742BDFC4A0A3C287009}

Om du vill lagra träffar när enheten är offline kan du aktivera spårning offline i [ADBMobileConfig.json-konfiguration](/help/windows-appstore/c-configuration/methods.md). Innan du aktiverar spårning offline bör du vara uppmärksam på de tidsstämpelkrav som beskrivs i config-filreferensen.
...
* Latitud/longitud och POI (om de ligger inom en POI som definieras i `ADBMobileConfig.json` konfigurationsfil). Dessa skickas till mobillösningsvariabler för automatisk rapportering.
...
När enhetens placering bedöms ligga inom en radie på 7000 meter från den definierade punkten, kan `a.loc.poi` kontextdatavariabeln med värdet &quot;San Francisco&quot; skickas in med `TrackLocation` träff. An `a.loc.dist` sammanhangsvariabel skickas med avståndet i meter från de definierade koordinaterna.
