...
If `state` är tom visas den som&quot;app name app version (build)&quot; i rapporter. Om du ser det här värdet i rapporter måste du ange `state` i varje `TrackState` ring.
...
* **TrackTimed &#x200B; ActionUpdate (winJS: trackTimed &#x200B; ActionUpdate)**
   ...
   Skicka in `contextData` för att uppdatera kontextdata som är associerade med den angivna `action`. The `data` som skickas läggs till i befintliga data för den angivna åtgärden och skriver över data om samma nyckel redan har definierats för `action`.
